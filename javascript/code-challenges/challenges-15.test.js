...
let biggerThanLuke = (arr) => {
  const strArray = [];
  arr.forEach(obj => +obj.mass > +starWarsData[0].mass ? strArray.push(obj.name) : null);
...
const sortBy = (property, arr) => {
  return typeof arr[0][property] === 'number' ? arr.sort((a, b) => a[property] - b[property]) : arr.sort((a, b) => a[property] !== b[property] ? a[property] < b[property] ? -1 : 1 : 0);
};
...
const detectTicTacToeWin = (board) => {
  let result = false;
  const winPositions = {
    top: [board[0][1], board[0][1], board[0][2]],
    mid: [board[1][0], board[1][1], board[1][2]],
    bot: [board[2][0], board[2][1], board[2][2]],
    ldiag: [board[0][0], board[1][1], board[2][2]],
    rdiag: [board[2][0], board[1][1], board[0][2]],
    row1: [board[0][0], board[1][0], board[2][0]],
    row2: [board[0][1], board[1][1], board[2][1]],
    row3: [board[2][0], board[2][1], board[2][2]]
  };
  const wins = Object.values(winPositions);
  wins.forEach(arr => {
    const joined = arr.join('');
    console.log(joined);
    joined === 'XXX' || joined === 'OOO' ? result = true : null;
  });
